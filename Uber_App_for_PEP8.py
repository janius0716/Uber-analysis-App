...
def load_and_setup_data():
    """Load the dataset and set up the Streamlit page."""
...
def analyze_passenger_fare_relationship(df):
    """Analyze the relationship between passenger count and fare amount."""
    st.header(
        'Question 1: Relationship between passengers and fare amount'
    )
    ...
    selected_passenger_count = st.slider(
        'Select passenger count range',
        passenger_count_min,
        passenger_count_max,
    ...
    plt.title(
        'Relationship between Number of Passengers '
        'and Average Fare Amount'
    )
...
def analyze_time_fare_relationship(df):
    """Analyze how different times of day affect fare amounts."""
    st.header('Question 2: How do different times of day affect fares?')
    ...
    selected_time_category = st.sidebar.radio(
        'Select Time Category',
        list(time_categories.keys())
    )

    # Get selected hours
    selected_hours = time_categories[selected_time_category]
    ...
    hourly_avg_fare = (
        filtered_df_hour.groupby('hour')['fare_amount']
        .mean()
        .reset_index()
    )
...
def analyze_fare_distribution(df):
    ...
    st.header('EDA - Fare Amount Distribution (Histogram)')
    ...
    selected_fare_category = st.sidebar.radio(
        'Select Fare Category',
        list(fare_categories.keys())
    )

    # Filter data
    filter_func = fare_categories[selected_fare_category]
...
def analyze_passenger_distribution(df):
    ...
    st.header('EDA - Passenger Count Distribution (Bar Chart)')
    plt.figure(figsize=(10, 6))
...
def main():
    ...
    analyze_passenger_fare_relationship(df)
    analyze_time_fare_relationship(df)
    analyze_fare_distribution(df)
